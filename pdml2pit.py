...
def transform_field(f, root, depth):
    ...
    try:
        ...
        if len(f.get("value")) == 2 and show == int(f.get("value"), 16):
            parsed = True
        if len(f.get("value")) == 8 and (struct.unpack(">I", binascii.unhexlify(f.get("value")))[0] == show or
                                         struct.unpack(">i", binascii.unhexlify(f.get("value")))[0] == show):
            parsed = True
            endian = "little"
        if len(f.get("value")) == 4 and (struct.unpack(">H", binascii.unhexlify(f.get("value")))[0] == show or
                                         struct.unpack(">h", binascii.unhexlify(f.get("value")))[0] == show):
            parsed = True
            endian = "little"
        if len(f.get("value")) == 8 and (struct.unpack("<I", binascii.unhexlify(f.get("value")))[0] == show or
                                         struct.unpack("<i", binascii.unhexlify(f.get("value")))[0] == show):
        ...
        if len(f.get("value")) == 4 and (struct.unpack("<H", binascii.unhexlify(f.get("value")))[0] == show or
            ...
            parsed = True
            endian = "big"
        if len(f.get("value")) == 2 and (struct.unpack("<B", binascii.unhexlify(f.get("value")))[0] == show or
                                         struct.unpack("<b", binascii.unhexlify(f.get("value")))[0] == show):
            parsed = True
            endian = "little"
        if parsed:
            node = etree.Element('Number')
